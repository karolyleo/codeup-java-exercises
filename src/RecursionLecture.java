...
//        }
    ...
    public static void printLetters(String aWord) {
        if (aWord.length() == 1) {
            System.out.print(aWord);
        } else {
            char currentLetter = aWord.charAt(0);
            System.out.println(currentLetter);
            printLetters(aWord.substring(1));
        }
    }
    ...
    public static int sum(int max, int helper, int total) {
        if (helper >= max) {
            System.out.printf("The total from 1 to %d is %d.%n", max, total);
            return total;
        } else {
            helper += 1;
            total += helper;
//            System.out.printf("userNum | CurrentEval | Total %n%-8d| %-12d| %d%n", max, helper, total);
            return sum(max, helper, total);
        }
    }

    // BONUS TODO: use recursion to write a factorial method (very similar to the third todo)
    public static void factorial(int max) {
        System.out.printf("%d! = ", max);
        factorial(max, 0, 1);
    }

    public static int factorial(int max, int helper, int total) {
        helper += 1;
        total *= helper;
        if (helper >= max) {
            System.out.printf("%d = %d%n", max, total);
            return total;
        } else {
            System.out.printf("%d x ", helper);
            return factorial(max, helper, total);
        }
    }

    // BONUS TODO: use recursion to reverse the characters in a string
    public static void reverseString(String orginalWord) {
        int max = orginalWord.length();
        reverseString(orginalWord, 0, max, "");
    }

    public static String reverseString(String oldWord, int index, int max, String newWord){
        index++;
        newWord += oldWord.charAt(max-index);
//        System.out.printf("%s  = %d%n",newWord, index); //used to help understand what is happening "under the hood"
        if(index>=max) {
            System.out.printf("The reverse of %s is %s", oldWord, newWord);
            return newWord;
        }
        return reverseString(oldWord, index, max, newWord);
    }

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        /**First TODO:*/
//        System.out.println("Choose a num to starting counting up to 100");
//        int userNum = sc.nextInt();
//        countUp(userNum);

        /** Second TODO: */
//        System.out.println("Choose a word to breakdown");
//        String breakdown = sc.nextLine();
//        printLetters(breakdown);

        /** This is the third TODO:*/
//        System.out.println("%nInput a num for a total sum");
//        int aNum = sc.nextInt();
//        sum(aNum);

        /** This is the first Bonus:*/
//        System.out.println("Input a num to factorize");
//        aNum = sc.nextInt();
//        factorial(aNum);

        /** This is the second Bonus:*/
